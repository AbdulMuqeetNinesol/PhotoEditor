...
import com.burhanrashid52.photoediting.Ads.getFilePAth
import com.burhanrashid52.photoediting.Ads.sendAnalytics
import com.burhanrashid52.photoediting.Ads.takeScreenshotOfView
...
import editphotos.backgroundremover.photobackgroundchanger.databinding.ActivityEditImageBinding
import editphotos.backgroundremover.photobackgroundchanger.databinding.MyCustomDialogTwoBinding
...
////            val bmp = takeScreenshotOfView(
////                mPhotoEditorView,
////                mPhotoEditorView.height,
////                mPhotoEditorView.width
////            )
////            saveToStorage(bmp,this)
//
...
//        mPhotoEditorView.source.setImageResource(R.drawable.paris_tower)
        mPhotoEditorView.source.setImageURI(FrameActivity.photoUri)
//        mPhotoEditorView.source.setImageURI(FrameActivity.photoUri)
    ...
    override fun onClick(view: View) {
        when (view.id) {
            ...
            R.id.imgSave -> {
                ...
                // Start the delayed task to start the activity after 3 seconds
                handler.postDelayed({
                    // Start the activity
                    val intent = Intent(this, savePhoto::class.java)
                    startActivity(intent)
...
//        startActivity(Intent.createChooser(intent, getString(R.string.msg_share_image)))
    ...
    private fun saveToStorage(bitmap: Bitmap){
        ...
        val result = mSaveImageUri

        val directory = Environment.getExternalStoragePublicDirectory(
            Environment.DIRECTORY_PICTURES + "/savephoto")
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (resultCode == RESULT_OK && requestCode == UCrop.REQUEST_CROP) {
            ...
            val croppedImage = resultUri?.toFile()
            FrameActivity.photoUri = resultUri!!

            Glide.with(this)
                .load(FrameActivity.photoUri)
                .centerCrop() // Set the scaling type to center crop
    ...
    override fun onToolSelected(toolType: ToolType) {
        when (toolType) {
            ToolType.SHAPE -> {
                sendAnalytics("EditImageActivity","shape_btn_click")
            ...
            ToolType.TEXT -> {
                sendAnalytics("EditImageActivity","text_btn_click")
            ...
            ToolType.ERASER -> {
                ...
                sendAnalytics("EditImageActivity","eraser_btn_click")
                mPhotoEditor.brushEraser()
                mTxtCurrentTool.setText(R.string.label_eraser_mode)
            }
            ToolType.CROP ->{
                sendAnalytics("EditImageActivity","crop_btn_click")
                val bmp= takeScreenshotOfView(binding.photoEditorView,binding.photoEditorView.height,binding.photoEditorView.width)
                val sourcePath = FrameActivity.photoUri

                val destinationFile = "%03d.jpg"
                val destinationPath = Uri.fromFile(File(destinationFile.getFilePAth(this)))
            ...
            ToolType.FILTER -> {
                sendAnalytics("EditImageActivity","filter_btn_click")
...
//                showBottomSheetDialogFragment(mStickerBSFragment)
    ...
    private fun showDialog() {
        ...
        val binding = MyCustomDialogTwoBinding.inflate(LayoutInflater.from(this))
        builder.setView(binding.root)
        val alertDialog: AlertDialog = builder.create()
        binding.ok.setOnClickListener {
            sendAnalytics("EditImageActivity","remove_BG_btn_click")
            val mProgressDialog = CustomProgressDialog(this)
            mProgressDialog.start("")
            handler.postDelayed({ mProgressDialog.stop() }, 5000)
            Toast.makeText(this, "Wait for few sec", Toast.LENGTH_LONG).show()
            removeBg()
            alertDialog.cancel()
        }
    ...
    private fun removeBg() {
        ...
        BackgroundRemover.bitmapForProcessing(
            binding.photoEditorView.source.drawable.toBitmap(),
            true,
            object : OnBackgroundChangeListener {
                ...
                override fun onFailed(exception: Exception) {
                    Toast.makeText(this@EditImageActivity, "Error Occur", Toast.LENGTH_SHORT).show()
                }
